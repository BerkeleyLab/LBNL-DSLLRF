...
class PulserAMOverlay(Overlay):
    def __init__(self, bitfile_name='pulser_am.bit', **kwargs):
        ...
        if board == 'ZCU208':
            xrfclk.set_ref_clks(lmk_freq = ZCU208_LMK_FREQ, lmx_freq = ZCU208_LMX_FREQ) # MHz
        ...
        self.mode_iq = True
        self.adc_captures = [
            ("hier_adc10_I_cap/axi_bram_ctrl_0", "hier_adc10_Q_cap/axi_bram_ctrl_0"),
            ("hier_adc20_I_cap/axi_bram_ctrl_0", "hier_adc20_Q_cap/axi_bram_ctrl_0"),
        ]
...
class PulseGen():
    ...
    def __init__(self, ip_dict_item, count_width=8, fclk=250.0e6, dwh=20, dwl=12):
        self._dwh = dwh
        self._dwl = dwl
    ...
    def get_dds(self):
        ...
        check_ratio = invert_dds(phase_step_h, phase_step_l, modulo, dwh=self._dwh, dwl=self._dwl)
        check_fdds = self._fclk*check_ratio
        return check_fdds

    def set_dds_freq(self, fdds):
        ph, pl, mod = get_dds_config(self._fclk, fdds, dwh=self._dwh, dwl=self._dwl)
...
def invert_dds(phase_step_h, phase_step_l, modulo, dwh=32, dwl=12):
    """Return num/den = f_dds/f_clk given a phase_step_h, phase_step_l, and modulo."""
