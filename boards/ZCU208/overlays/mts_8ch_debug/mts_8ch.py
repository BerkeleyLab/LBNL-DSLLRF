...
class mtsOverlay(Overlay):
    ...
    def __init__(self, bitfile_name="mts.bit", **kwargs):
        ...
        dts = pynq.DeviceTreeSegment(resolve_binary_path(
                                        DEVICETREE_OVERLAY_FOR_PLDRAM))
        if not dts.is_dtbo_applied():
            dts.insert()
        # must configure clock synthesizers
        # the LMK04828 PL_CLK and PL_SYSREF clocks
        if board == "RFSoC4x2":
            xrfclk.set_ref_clks(lmk_freq=RFSOC4X2_LMK_FREQ,
                                lmx_freq=RFSOC4X2_LMX_FREQ)
    ...
    def verify_clock_tree(self):
        ...
        # refer to PG065
        if Xstatus != 1:
            raise Exception("The MTS ClockTree has failed to LOCK."
    ...
    def internal_capture(self, membuffer):
        """Captures ADC samples from three channels
...
# -------------------------------------------------------------------------------------------------
# Convenience API for DebugBridge
def GetDebugBridge(base_addr, high_addr):
    """PYNQ doesn't yet support the DebugBridge IP from the HWH file parser.
    Instead, you'll need to grab the BASEADDR and HIGHADDR values either from
    the .hwh file or from the Vivado Address Editor and manually instantiate.
    The important API for the bridge are:
    ...
    return DebugBridge(ReprDict(dd))


def StartServer(bridge, port=2542):
    return bridge.start_xvc_server(serverPort=port)


def StopServer(bridge):
    if bridge.serverThread is not None:
        bridge.stop_xvc_server()
...
def GetServerPort(bridge):
    if bridge.serverThread is not None:
        print(bridge.serverThread.serverPort)
        return bridge.serverThread.serverPort
    else:
...
# -------------------------------------------------------------------------------------------------
# Custom class for diag_readback.v
