# ALS LLRF MTS Overlay

## Features

* Multi-Tile Synchronization
* 8 ADC, 8 DAC fully enabled
* Clocks
  * 7.0 Gsps DAC sampling rate
  * 3.5 Gsps ADC sampling rate
  * 437.5 MHz fabric clock (8 samples per cycle)
  * Externally referenced to 437.5MHz PL_CLK clock by CLK104 J11
  * Generated by CLK104 LMX
  * Aligned by 1.25MHz PL_SYSREF clock by CLK104 LMK
* Digital mixer
  * NCO mixer (IQ) digital up/down conversion
  * NCO phase aligned
* 64k samples buffer for:
  * Arbitrary waveform generator, broadcasted to all DACs
  * I and Q waveforms for each ADC
* MRF Timing Event Receiver (EVR)
  * Through zSFP 2
  * Self-recovery from timing link interruptions
  * Deterministic latency
  * Generate event flags by subscribing an event code
  * Arbitrary GTY reference frequency generation via Silicon Lab SI570 (124.91MHz)
* Trigger
  * Multiplex trigger source between software, internal and EVR
  * Synchronous trigger to all DACs / ADCs

## Status

* [x] ADC / DAC waveforms
* [x] MTS
* [x] NCO phase alignment
* [x] Trigger source multiplex
* [] Determinisitc Latency
