#-----------------------------------------------------------------------------
# Vivado TCL Script: Synthesize axil_wave_gen and Report Utilization
#
# Usage:
#   vivado -mode batch -source synth.tcl
#   vivado -mode batch -source synth.tcl -tclargs -part xczu48dr-fsvg1517-2-e
#   vivado -mode batch -source synth.tcl -tclargs -axi_width 32 -out_width 320
#
...
# Project settings
set project_name "axil_wave_gen_synth"
set project_dir "./vivado_synth"
set source_file "../../rtl/wave_gen.v"
set wrapper_file "../../rtl/axil_wave_gen.v"
set top_module "axil_wave_gen"
...
proc parse_args {} {
    global argc argv
    global part axi_data_width axi_addr_width out_data_width granule bank_depth read_latency ram_style
    ...
    for {set i 0} {$i < $argc} {incr i} {
        ...
        switch -exact -- $arg {
            ...
            "-axi_addr_width" {
                incr i
                set axi_addr_width [lindex $argv $i]
            }
            "-out_width" {
                incr i
...
proc print_usage {} {
    ...
    puts "  -axi_width <bits>     AXI data width: 32 or 64 (default: 32)"
    puts "  -axi_addr_width <bits>AXI address width (default: 6)"
    ...
    puts "  -read_latency <n>     Read pipeline latency (default: 3)"
    puts "  -ram_style <style>    RAM style: ultra or block (default: ultra)"
    puts "  -project_name <name>  Project name (default: axil_wave_gen_synth)"
    ...
    puts ""
    puts "Examples:"
    puts "  # Default 32->320 configuration"
    puts "  vivado -mode batch -source synth.tcl"
    puts ""
    puts "  # 64->448 configuration"
    puts "  vivado -mode batch -source synth.tcl -tclargs -axi_width 64 -out_width 448"
    puts ""
...
proc run_synthesis {} {
    ...
    puts ""
    puts "=============================================================================="
    puts " AXI-Lite Waveform Generator Synthesis"
    puts "=============================================================================="
    puts " Part:           $part"
    puts " AXI Width:      $axi_data_width bits"
    puts " AXI Addr Width: $axi_addr_width bits"
    ...
    # Validation: AXI4-Lite data width limited to 32 or 64 bits
    if {$axi_data_width != 32 && $axi_data_width != 64} {
        puts "ERROR: AXI4-Lite data width must be 32 or 64. Provided: $axi_data_width"
        exit 1
    }

    if {[expr {$axi_data_width % $granule}] != 0} {
        puts "ERROR: AXI width ($axi_data_width) must be a multiple of granule ($granule)"
        exit 1
    }

    if {[expr {$out_data_width % $granule}] != 0} {
        puts "ERROR: Output width ($out_data_width) must be a multiple of granule ($granule)"
        exit 1
    }
    ...
    # Set generics for the top module
    set generics "C_S_AXI_DATA_WIDTH=$axi_data_width C_S_AXI_ADDR_WIDTH=$axi_addr_width OUT_DATA_WIDTH=$out_data_width BANK_DEPTH=$bank_depth GRANULE=$granule READ_LATENCY=$read_latency RAM_STYLE=$ram_style"
    puts "INFO: Setting generics: $generics"
...
proc run_multi_config_test {} {
    ...
    # Format: {axi_w addr_w out_w granule ram_s description}
    set configs {
        {32 16 320 32 ultra "32b AXI-Lite -> 320b Out (UltraRAM)"}
        {32 16 448 32 ultra "32b AXI-Lite -> 448b Out (UltraRAM)"}
        {64 16 320 32 ultra "64b AXI-Lite -> 320b Out (UltraRAM)"}
        {64 16 448 64 ultra "64b AXI-Lite -> 448b Out (UltraRAM)"}
        {32 16 320 32 block "32b AXI-Lite -> 320b Out (BlockRAM)"}
        {64 16 320 64 block "64b AXI-Lite -> 320b Out (BlockRAM)"}
    }
    ...
    foreach config $configs {
        ...
        set addr_w [lindex $config 1]
        set out_w [lindex $config 2]
        set gran  [lindex $config 3]
        set ram_s [lindex $config 4]
        set desc  [lindex $config 5]
        ...
        global axi_data_width axi_addr_width out_data_width granule ram_style project_name
        set axi_data_width $axi_w
        set axi_addr_width $addr_w
        set out_data_width $out_w
        set granule        $gran
        set ram_style      $ram_s
        set project_name "axilwfg_${axi_w}_${out_w}_${ram_s}"
